...
A **Pickup** represents a parent's intent to pick up one or more of their children from school. Parents initiate a pickup session, which is placed into a school-managed queue. The pickup progresses through lifecycle stages from `active` → `prepare` → `queued` → `completed` (or `cancelled`). Redis manages the real-time queue; PostgreSQL (via Ent ORM) is the source of truth for persistence.

## Functional Requirements

- `primary_parent` and `additional_parent` can **create**, **read**, **update**, and **cancel** a pickup
- On creation, the parent selects one or more enrolled children and an approved vehicle (or chooses walking mode)
- On update, the parent sends GPS position; backend calculates stage transition and optionally associates a device ID
...
```
active(0) → prepare(1) → queued(2) → completed(3)
                  ↘           ↘
                            cancelled(-1)
```
...
| ----- | ----- | ------- | ----------- |
| `active` | 0 | Pickup created by parent | Created; parent has not approached school yet |
| `prepare` | 1 | GPS within **500 m** of school | Parent is very close; school/staff are alerted |
| `queued` | 2 | `device_id` received matching a registered gate unit | Parent at the gate; student being released |
| `completed` | 3 | Explicit completion call / school-side confirmation | Student successfully handed over |
...
Pickup supports three modes of arrival:

| Type | `vehicle_type` value | Vehicle Required | Notes |
| ---- | --------------------- | ---------------- | ----- |
| Private vehicle | `"private"` | Yes — must be an approved family vehicle | Standard parent pickup |
| Taxi / Shared vehicle | `"taxi"` | Yes — must be an approved shared vehicle (`owner_id IS NULL`) | Third-party pickup |
| Walking | `"walking"` | No | Parent arrives on foot |
...
sequenceDiagram
    ...
    p->>h: POST /api/v1/me/pickup<br/>{vehicle_type, vehicle_id?, student_ids, notes?}
    h->>h: parse JWT → user_id, family_id, school_id
    h->>s: CreatePickup(ctx, userID, req)
    alt vehicle_type == "private"
        s->>db: verify vehicle belongs to family & is approved & type=private
    else vehicle_type == "taxi"
        s->>db: verify vehicle is approved shared vehicle & type=taxi
    else vehicle_type == "walking"
        s->>db: vehicle_id = uuid.Nil (no validation)
    else
        s-->>h: ErrInvalidVehicleType
        h-->>p: 400
    end
    ...
    s->>db: INSERT pickup (stage=0/active)
...
sequenceDiagram
    ...
    s->>s: calculate Haversine distance to school (hardcoded coords)
    alt stage == active AND distance ≤ 500m
        s->>db: UPDATE stage=prepare, current_lat, current_lng
    else stage == prepare AND device_id provided
        s->>db: UPDATE stage=queued, current_lat, current_lng
    else
...
| ------ | ---- | ----- | ----------- |
| `GET` | `/available-children` | `primary_parent`, `additional_parent` | List enrolled children in family not in active pickups |
| `GET` | `/available-vehicles` | `primary_parent`, `additional_parent` | List approved private vehicles in family |
| `GET` | `/shared-vehicles` | `primary_parent`, `additional_parent` | List approved shared vehicles (taxi type) |
| `GET` | `/` | `primary_parent`, `additional_parent` | List my active pickups (stages 0, 1, 2) |
| `GET` | `/{id}` | `primary_parent`, `additional_parent` | Get pickup detail |
| `GET` | `/{id}/queue` | `primary_parent`, `additional_parent` | Get queue position and ETA |
| `POST` | `/` | `primary_parent`, `additional_parent` | Create new pickup |
| `PUT` | `/{id}` | `primary_parent`, `additional_parent` | Update pickup position / stage |
| `DELETE` | `/{id}` | `primary_parent`, `additional_parent` | Cancel pickup |

## Domain Models

```go
// Pickup — stage values: -1=cancelled 0=active 1=prepare 2=queued 3=completed
type Pickup struct {
    ID          uuid.UUID  `json:"id"`
    SchoolID    uuid.UUID  `json:"school_id"`
    FamilyID    uuid.UUID  `json:"family_id"`
    CreatedBy   uuid.UUID  `json:"created_by"`
    VehicleID   uuid.UUID  `json:"vehicle_id"` // uuid.Nil for walking mode
...
type CreatePickupRequest struct {
    VehicleType string      `json:"vehicle_type"` // "private", "taxi", "walking"
    VehicleID   uuid.UUID   `json:"vehicle_id"`   // uuid.Nil for walking mode
...
| `vehicle_id` | `uuid` | NOT NULL | Vehicle used for this pickup (uuid.Nil for walking) |
...
| `ErrInvalidVehicleType` | 400 | `40008` | invalid vehicle type (must be private, taxi, or walking) |
...
type ParentAppService interface {
    ...
    GetAvailableVehicles(ctx context.Context, familyID string) ([]domain.Vehicle, error)
    GetSharedVehicles(ctx context.Context) ([]domain.Vehicle, error)
